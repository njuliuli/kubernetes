...
func TestCgroupCPUSetAddPod(t *testing.T) {
	testCaseArray := []struct {
		...
		{
			description: "Fail, mode=modeCPUSetUnknown",
			ccsBefore:   testGenerateCgroupCPUSet(&testCgroupCPUSet{}),
			pod:         testGeneratePodCPUSet("1", "", ""),
			mode:        modeCPUSetUnknown,
			ccsAfter: testGenerateCgroupCPUSet(&testCgroupCPUSet{
				podSet: sets.NewString("1"),
			}),
			expErr: fmt.Errorf("fake error"),
		},
		// For modeCPUSetDefault, the pod is tracked, but cgroupCPUSet is not changed
		{
			description: "Success, mode=modeCPUSetDefault, state unchanged",
			ccsBefore:   testGenerateCgroupCPUSet(&testCgroupCPUSet{}),
			pod:         testGeneratePodCPUSet("1", "", ""),
			mode:        modeCPUSetDefault,
			ccsAfter: testGenerateCgroupCPUSet(&testCgroupCPUSet{
				podSet: sets.NewString("1"),
			}),
		},
		...
		// For modeCPUSetDedicated, quantity in pod matters.
		// Tests below is based on default cgroupCPUSet generated by testGenerateCgroupCPUSet above
		{
			description: "Fail, mode=modeCPUSetDedicated, but pod needs 0 dedicated CPUs (request=limit, empty)",
			ccsBefore:   testGenerateCgroupCPUSet(&testCgroupCPUSet{}),
			pod:         testGeneratePodCPUSet("1", "", ""),
			mode:        modeCPUSetDedicated,
			ccsAfter: testGenerateCgroupCPUSet(&testCgroupCPUSet{
				podSet: sets.NewString("1"),
			}),
			expErr: fmt.Errorf("fake error"),
		...
		{
			description: "Fail, mode=modeCPUSetDedicated, pod is too large to fit into this host",
			ccsBefore:   testGenerateCgroupCPUSet(&testCgroupCPUSet{}),
			pod:         testGeneratePodCPUSet("1", "100", "100"),
			mode:        modeCPUSetDedicated,
			ccsAfter: testGenerateCgroupCPUSet(&testCgroupCPUSet{
		...
		{
			description: "Success, mode=modeCPUSetDedicated, pod with (request=limit, int, thread x1)",
			ccsBefore:   testGenerateCgroupCPUSet(&testCgroupCPUSet{}),
		...
		{
			description: "Success, mode=modeCPUSetDedicated, pod with (request=limit, int, thread x2 -> core x1)",
			ccsBefore:   testGenerateCgroupCPUSet(&testCgroupCPUSet{}),
		...
		{
			description: "Success, mode=modeCPUSetDedicated, pod with (request=limit, int, thread x12 -> node x1)",
			ccsBefore:   testGenerateCgroupCPUSet(&testCgroupCPUSet{}),
